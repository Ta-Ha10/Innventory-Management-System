...
class _InventoryPageState extends State<InventoryPage>
  ...
  Future<void> _saveSupplier(String name, String itemName) async {
    ...
    final fakePhone = '+1-${Random().nextInt(900) + 100}-${Random().nextInt(9000) + 1000}';
  ...
  Future<void> _saveItem() async {
    if (_selectedItemId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No item selected')));
      return;
    }
    ...
    if (name.isEmpty || category.isEmpty || supplier.isEmpty || unit.isEmpty || price == null) {
    ...
    final data = {
      'name': name,
      'category': category,
      'quantity': quantity ?? _parseQuantity(_localDocs[_selectedItemId!]?['quantity'] ?? 0),
      'unit': unit,
      'supplier': supplier,
      'price': price,
  ...
  Future<void> _saveNewItem() async {
    ...
    try {
      final ref = await FirebaseFirestore.instance.collection('raw_components').add(data);
      final realId = ref.id;
  ...
  Future<void> _showAddQuantityDialog(String? id, Map<String, dynamic> item) async {
    if (id == null) return;
    ...
    final result = await showDialog<int?>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(hintText: 'Enter quantity to add (e.g., 5 or 2.5)'),
    ...
    // compute new quantity using local cache or item value
    final current = _parseQuantity(item['quantity']);
    final newQty = current + added;

    // optimistic local update
    _localDocs[id] = {...item, 'quantity': newQty, 'id': id};
    setState(() {});

    // If the id looks like a temporary local id (starts with local_), try to just keep local cache.
    ...
    try {
      await FirebaseFirestore.instance.collection('raw_components').doc(id).update({'quantity': newQty});
  ...
  Widget _buildSupplierField(TextEditingController controller, {bool isAdd = false}) {
    // Show a dropdown populated from _supplierCache (loaded from Firestore).
    final items = _supplierCache.map((s) => DropdownMenuItem<String>(value: s, child: Text(s))).toList();
    final value = controller.text.isNotEmpty && _supplierCache.contains(controller.text) ? controller.text : null;
    return DropdownButtonFormField<String>(
      ...
      validator: (v) => (v == null || v.isEmpty) ? 'Supplier is required' : null,
    );
  }
            ...
            ValueListenableBuilder(
              ...
              builder: (context, unit, _) {
                return TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Price/${unit.text.isNotEmpty ? unit.text : "unit"} *',
                    hintText: 'e.g., 12.50',
              ...
              builder: (context, unit, _) {
                return TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Price/${unit.text.isNotEmpty ? unit.text : "unit"} *',
                    hintText: 'e.g., 12.50',
                    suffixText: _addUnitController.text.isNotEmpty ? '/${_addUnitController.text}' : '/unit',
                    border: const OutlineInputBorder(),
                  ),
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                );
  ...
  final double sidebarWidth = isCompact ? 120 : (isTablet ? 180 : 220);
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          // Open the add panel and clear any selection
          setState(() {
            _clearSelection();
            _clearAddFormFields();
            _showAddPanel = true;
                                ...
                                builder: (context, selectedIndex, _) {
                                  return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                                    ...
                                    builder: (context, snapshot) {
                                      ...
                                      final firestoreDocs = {
                                        for (var d in allDocs) d.id: {...d.data(), 'id': d.id}
                                      };
                                      ...
                                      if (selectedIndex > 0 && selectedIndex < categories.value.length) {
                                        final normSelected = categories.value[selectedIndex]
                                            .toLowerCase()
                                            .replaceAll(RegExp(r"\s+|_|-|/|&"), ' ')
                                            .trim();
                                          ...
                                          cells: [
                                            ...
                                            DataCell(CircleAvatar(
                                              ...
                                              backgroundColor: quantityInt <= 5
                                                  ? Colors.red
                                                  : (quantityInt <= 10 ? Colors.yellow[700]! : Colors.green),
                                            )),
                                            DataCell(
                                              IconButton(
                                                icon: const Icon(Icons.add_circle, size: 18, color: Color(0xff4a4a4a)),
                                                tooltip: 'Add quantity',
                                                onPressed: () => _showAddQuantityDialog(id, displayed),
                                              ),
                                            ),
                                            DataCell(
                                              IconButton(
                                                ...
                                                onPressed: () {
                                                  ...
                                                  _priceController.text = displayed['price'] is num
                      ...
                      children: [
                        ...
                        AnimatedSwitcher(
                          ...
                          child: _selectedItemId != null
                              ? editPanel()
                              : SizedBox(key: const ValueKey('empty_edit_panel'), height: 0),
                        ),
                        const SizedBox(height: 12),
                        AnimatedSwitcher(
                          duration: const Duration(milliseconds: 220),
                          child: _showAddPanel ? addPanel() : SizedBox(key: const ValueKey('empty_add_panel'), height: 0),
                        ),
                      ],
                    ),
                  ),
